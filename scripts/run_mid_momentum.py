#!/usr/bin/env python3
"""Run the mid-momentum runner from the standalone suite."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Vector mid-momentum engine from this suite")
    parser.add_argument("--market", default="brent_runner", help="Mid-momentum market profile name")
...
def main() -> int:
    ...
    profile_file = ROOT / "config" / "mid_momentum" / f"{args.market}.json"
    ...
    write_manifest(
        ...
        strategy_name="Vector Momentum Engine",
        market_name=profile_config["market_name"],
        account_name=args.account,
        account_file=account_file,
        profile_file=profile_file,
        bot_script="pa_momo_mid_bot_pro.py",
        dashboard_script="pa_momo_mid_dashboard_pro.py",
    ...
    return run_managed(
        ...
        title=f"Starting Vector Momentum Engine for {profile_config['market_name']}",
    )
