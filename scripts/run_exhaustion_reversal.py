#!/usr/bin/env python3
"""Run the exhaustion-reversal engine from the standalone suite."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the exhaustion-reversal engine from this suite")
    parser.add_argument("--market", default="brent", help="Exhaustion-reversal market profile name")
...
def main() -> int:
    ...
    profile_file = ROOT / "config" / "exhaustion_reversal" / f"{args.market}.json"
    ...
    paths = make_run_paths(
        strategy_slug="exhaustion_reversal",
    ...
    write_manifest(
        ...
        strategy_name="Exhaustion Reversal Engine",
        market_name=profile_config["market_name"],
        account_name=args.account,
        account_file=account_file,
        profile_file=profile_file,
        bot_script="pa_exhaustion_bot_pro.py",
        dashboard_script="pa_exhaustion_dashboard_pro.py",
    ...
    return run_managed(
        ...
        title=f"Starting Exhaustion Reversal Engine for {profile_config['market_name']}",
    )
