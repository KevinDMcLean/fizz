#!/usr/bin/env python3
"""Run the rare-event momentum engine from the standalone suite."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the extreme momentum engine from this suite")
    parser.add_argument("--market", default="brent", help="Extreme-momentum market profile name")
...
def main() -> int:
    ...
    profile_file = ROOT / "config" / "extreme_momentum" / f"{args.market}.json"
    ...
    paths = make_run_paths(
        strategy_slug="extreme_momentum",
        market_slug=args.market,
        run_name=run_name,
        include_markouts=False,
    )
    ...
    write_manifest(
        ...
        strategy_name="Extreme Momentum Engine",
        market_name=profile_config["market_name"],
        account_name=args.account,
        account_file=account_file,
        profile_file=profile_file,
        bot_script="pa_pump_bot_pro.py",
        dashboard_script="pa_pump_dashboard_pro.py",
    ...
    return run_managed(
        ...
        title=f"Starting Extreme Momentum Engine for {profile_config['market_name']}",
    )
